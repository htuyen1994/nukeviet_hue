...
//lấy về biến listcatid kiểm tra nếu có trang catalog thì gán điều kiện bảng rows listcatid=subcatid
...
if (!empty($listcatid)) {
	if (isset($global_array_shops_cat[$listcatid])) {
		...
		if ($subcatid != 0) //nếu k có listcatid thì k lấy subcatid
		{
...
$array_search = array('product_code' => $lang_module['search_product_code'], 'title' => $lang_module['search_title'], 'bodytext' => $lang_module['search_bodytext'], 'author' => $lang_module['search_author'], 'admin_id' => $lang_module['search_admin']);
$array_in_rows = array('title', 'bodytext');
$array_in_ordername = array('title', 'publtime', 'exptime', 'hitstotal', 'product_number', 'num_sell');
...
//gán lại điều kiện cho biên form
if (!empty($where)) {
